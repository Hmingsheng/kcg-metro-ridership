...
export function MonthlyStationExplorer({ records }: { records: RidershipRecord[] }) {
  ...
  return <section aria-label="單站月度趨勢">
    ...
    {stationName && <>
      <h3>{stationName}月度入站趨勢</h3>
      <table><thead><tr><th scope="col">月份</th><th scope="col">入站人次</th></tr></thead><tbody>
        {trend.map((record) => <tr key={record.period}><td>{record.period.slice(0, 4)} 年 {Number(record.period.slice(5))} 月</td><td>{record.passengers.toLocaleString('zh-TW')}</td></tr>)}
      </tbody></table>
    </>}
  </section>;
}
